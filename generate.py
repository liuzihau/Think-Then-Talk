...
from utils import topk_soft_embedding_from_logits
...
def generate_with_prefix_cache(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        block_mask_index = (x[:, :current_block_end] == mask_id)
        num_transfer_tokens = get_num_transfer_tokens(block_mask_index, steps)

        nfe += 1
        output = model(x[:, :current_block_end], use_cache=False)  # [B, P+nL]

        x_talk = x[:, :current_block_end][block_mask_index].view(x.shape[0], -1)  # [B, L]
        think_rps = output.hidden_states[block_mask_index].view(output.hidden_states.shape[0], -1, output.hidden_states.shape[2])  # [B, L]
        output= model(input_ids=x_talk,inputs_repres=think_rps, use_cache=False)  # [B, L]

        mask_index = (x_talk == mask_id)  # [B, L]
        if factor is None:
            x0, transfer_index = get_transfer_index(output.logits, temperature, remasking, mask_index, x_talk, num_transfer_tokens[:, 0] if threshold is None else None, threshold)
        else:
            x0, transfer_index = get_transfer_index_dynamic(output.logits, temperature, remasking, mask_index, x, None, factor)
        x_talk[transfer_index] = x0[transfer_index]

        i = 1
        while True:
            if (x_talk == mask_id).sum() == 0:
                break
            nfe += 1
            mask_index = (x_talk == mask_id)
            mask_index[:, block_length:] = 0

            base_emb = F.embedding(x_talk, model.talk_embed_weight).to(dtype=model.talk_embed_weight.dtype) 
            soft_emb = topk_soft_embedding_from_logits(
                    logits=output.logits,
                    emb_weight=model.talk_embed_weight,
                    topk=32,
                    temperature=1.5,
                ).to(dtype=model.talk_embed_weight.dtype) 
            x_emb = torch.where(transfer_index.unsqueeze(-1), soft_emb, base_emb)
            
            output = model(input_ids=None, inputs_embeds=x_emb, inputs_repres=output.hidden_states, use_cache=False)  # [B, L]

            if factor is None:
                x0, transfer_index = get_transfer_index(output.logits, temperature, remasking, mask_index, 
                                                x_talk, num_transfer_tokens[:, i] if threshold is None else None, threshold)
            else:
                x0, transfer_index = get_transfer_index_dynamic(output.logits, temperature, remasking, mask_index, 
                                                x[:, current_block_start:], None, factor)
            x_talk[transfer_index] = x0[transfer_index]
        
            i += 1
        x[:, :current_block_end][block_mask_index] = x_talk
