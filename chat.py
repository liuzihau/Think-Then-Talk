...
def chat(args):
    ...
    model = T3Model(model_config, think_dev=THINK_DEVICE, talk_dev=TALK_DEVICE, train=False).eval()
    load_ckpt(model_config["state_dir"], model)
    ...
    while True:
        ...
        input_ids = torch.tensor(input_ids).to(THINK_DEVICE).unsqueeze(0)
